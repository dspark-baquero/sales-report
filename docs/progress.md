# 개발 진행사항

## 현재 상태

- **배포**: Google Cloud Run (asia-northeast3) — `git push` → Cloud Run 소스 기반 자동 배포
- **데이터**: Google BigQuery (`sales` 테이블, 173,350행, 2023-07 ~ 현재)
- **목표**: BigQuery (`targets` 테이블, dataset `dashboard_1`)
- **인증**: Auth.js v5 + Google OAuth (`@baquero.co.kr` 전용)
- **첫 보고 기준월**: 2026-04
- **URL**: https://baquero-sales-report-koo6cv5uba-du.a.run.app
- **탭 수**: 13개 (종합/목표달성/해외영업/B2B종합/B2B/대리점/바크로하우스/B2C/면세점/브랜드분석/거래처분석/심층분석/비매출출고)
- **접근 제한**: B2B종합 탭은 화이트리스트(`config/access.ts`) 9명만 열람 가능, B2B 포함 그 외 탭은 `@baquero.co.kr` 전체 공개
- **숨김 페이지**: 영업사원 상세(`/sales-rep`)는 B2B종합에서 이름 클릭으로만 접근 (별도 메뉴 없음, B2B종합과 동일 권한)
- **데이터 새로고침**: 관리자(`isAdmin`)는 헤더 버튼으로 BigQuery 인메모리 캐시를 무효화 → 재배포 없이 즉시 반영. 토큰 보호 엔드포인트 `/api/refresh`도 제공(스크립트/자동화용)
- **접속 로깅**: 로그인 시 이메일·시각을 구조화 JSON으로 기록 (Cloud Logging `jsonPayload.event="login"`)

---

## 버전 이력

### v6.2 (2026-07-07)
채널 탭 "이번분기 누적" 카드 수치 오류 수정.
- **"이번분기 누적" 비교행 정정** — B2B·B2C·대리점·바크로하우스·면세점·수출·브랜드 탭의 실매출 카드에서 "이번분기 누적" 행이 실제로는 **이번달 실매출 vs 직전 분기 전체 3개월 누적**을 비교해 수치가 왜곡되던 문제. `MetricCard` 비교 항목에 선택적 `current`를 추가해, 이 행만 **이번분기 누적(kCurQ, 분기시작~이번달)** 을 기준·표시값으로 쓰고 **직전 분기 동기간(첫 qProg개월 누적)** 과 비교하도록 정정. 전분기 로딩도 전체 3개월 → 동기간(`ymMinusMonths(prevQ.qEnd, 3 - qProg)`)으로 변경. 계산만 하고 미사용이던 `kCurQ`(unused 경고)도 함께 해소

### v6.1 (2026-07-07)
월별 매출 추이 차트에 다음달 "전망" 칸 추가.
- **다음달 전망 칸(목표·전년)** — 모든 탭 공통 `YearToDateChart`에 연내 다음 달 칸을 하나 더 표시. 실매출 막대 없이 그 달의 **월별 목표(다이아)·전년 동기(회색 원)** 마커만 그리고 x축 라벨 아래 "전망" 표기. 이번달이 6월이면 아직 매출이 없는 7월의 목표·작년 실적을 미리 확인. 오버레이 배열이 경과월+1 길이면 차트가 전망 칸을 자동 감지(호출부 JSX 변경 최소화). 누적 달성률·전년 누적합은 경과월 기준 유지(전망 슬롯 제외), 12월이면 다음달이 내년이라 전망 칸 생략. 목표·전년 오버레이를 쓰는 전 탭 자동 적용(종합/B2B/대리점/바크로하우스/B2C/면세점/수출/B2B종합/영업사원/거래처). `lib/ytd.ts`(`ytdMonthsWithOutlook` + `outlook` 옵션)·`lib/compare.ts`(`nextMonthInYear`)·`components/YearToDateChart.tsx`·`components/charts/BarChart.tsx`(합계 0 라벨 숨김 + `muted` 톤)

### v6.0 (2026-07-06)
목표 매칭 정확화(돈키호테 벤더사·이중집계) + B2B종합 집계 정합 + 임직원 B2B 매출 정책 + 동면복귀 오분류 수정.
- **링커 제이와이(JY)메디코스 추가** — `config/mappings.ts` `LINKERS`에 추가(담당 류현희). B2B종합 영업사원별 통합실적에서 이 업체 매출이 류현희 링커란에 귀속. 기존 Harinbeauty와 동일 구조
- **목표달성탭 일본(돈키호테) 벤더사 매칭** — 돈키호테 매출은 벤더사 K-Labo(제이랩)를 경유해 거래처명이 "돈키호테"가 아니라 벤더사명으로 들어옴. 수출 카테고리 내 거래처명 K-Labo/제이랩(또는 돈키호테 직접 표기)을 돈키호테로 집계. `isDonkiSale` 단일 판정 함수로 통일(`lib/targets.ts`)
- **일본 이중집계 제거** — 일반 "일본" 목표 규칙이 country=일본 전체를 잡아 돈키호테 매출이 일본·돈키호테에 이중 계상되던 문제. 일반 일본 규칙에서 `isDonkiSale` 행을 제외
- **B2B종합 카드-차트 정합** — 카드 "B2B(직거래처+대리점+링커)"가 `repRows` 재조립(전 채널 `byMonthCustomer` + 대표유형)으로 계산돼 월별 매출추이 차트와 미세하게 어긋나던 문제. 카드를 `byMonthCategory["B2B"]`(=차트 스택 합) 단일 소스로 변경해 정확히 일치(`app/b2b-summary/page.tsx`)
- **임직원 B2B 실판매 매출 집계** — 사업형태 `임직원`은 실매출이 있어도 비매출 출고로 전량 제외되던 것을, **B2B몰 채널 + 실매출>0**에 한해 매출로 집계(정책 #2 예외, 2026-07 확정). 타 채널·실매출0 임직원은 종전대로 제외. `lib/parsers.ts` `isNonRev` + CLAUDE.md 문서화
- **동면 복귀 신규거래처 오분류 수정** — `sleepingReturned`가 "최근 N개월 0원 → 이번달 매출"만 보고 과거 이력이 전혀 없는 신규 거래처를 "12개월 만에 복귀"로 오분류. 전체 이력에서 마지막 활성월을 탐색하고, 과거 이력이 없으면 동면 복귀에서 제외(신규 거래처는 신규 진입 항목이 담당). `silentMonths`도 lookback에 갇히지 않고 실제 공백 반영(`lib/accountAnalysis.ts`)

### v5.9 (2026-06-10)
임원 비교 정보 강화 — 전년 대비 페이스 + 상위 거래처 확대 + 달성 구간 세분화.
- **목표 달성 상태 구간 세분화** — 기존 4구간에 '미달'(70~85%)을 추가해 5구간으로: 심각 미달 <70%(빨강) / 미달 70~85%(주황) / 정상 진행 85~95% / 근접 95~100% / 초과 ≥100%(녹색). `buildAchievement` 단일 소스로 전 화면(YTD 패널·게이지·카드·표) 일관 적용, 게이지 호 색상 경계도 85%로 일치
- **누적 실적 패널 전년 동기 비교 추가** — `YTDAchievementPanel`에 전년 동기 누적 매출 대비 비교(절대값+차이금액+증감률)를 추가해 목표뿐 아니라 작년 페이스와도 대조
- **종합탭 채널별 카드에 전년 동월 대비 추가** — 6채널 KPI 카드(`MetricCard`)에 전월·목표에 더해 전년 동월 매출 비교 노출
- **종합탭 상위 거래처 10→20개 확대** — `topNCustomersWithPrev` 호출을 20으로
- **목표달성탭 연 목표 진도 채널별 항상 노출** — 펼치기 토글 제거 후 전체+6채널 카드 기본 노출로 전환. 각 카드 하단에 작년 동기간 누적 대비 페이스(절대값+차이+증감률) 표시. 불필요해진 `AnnualProgressSection` 클라이언트 컴포넌트 삭제
- **B2B탭 상위 20 거래처 카드 추가** — 신규 거래처 위에 종합탭과 동일 형식(순위·거래처 링크·이번달/전월 실매출·차이·변화율)의 상위 20 거래처 표 추가, B2B 정책에 맞춰 대리점 제외(`b2bNonAgencyRows`)

### v5.8 (2026-06-05)
영업사원 상세 페이지 신설 + 목표달성 공식몰 기준 일관화 + 관리자 데이터 새로고침 + 월별 달성률 표기.
- **영업사원 상세 페이지(`/sales-rep`) 신설** — B2B종합에서 영업사원 이름 클릭 시 그 1명의 상세로 이동(별도 메뉴 없음, B2B종합과 동일 권한 `canAccessB2BSummary`). 소스별 순차 섹션(직거래처→바크로하우스→대리점→링커) + 공통 `YearToDateChart`(올해 월별 실적·목표·달성률) + 목표 게이지(월·누적) + 거래처유형 도넛 + 신규/이탈 거래처. 거래처명은 `/accounts`로 연결. `lib/salesRepProfile.ts`(기존 함수 조합, raw 스캔 없음) + `components/SalesRepLink.tsx`(링커/미지정은 일반 텍스트). B2B종합의 영업사원 이름이 나오는 모든 표(직거래처 `PerfTable` 포함) 클릭 가능화
- **목표달성 공식몰 기준을 B2C 탭과 통일** — 공식몰 목표 실적이 `BRAND_OFFICIAL_CHANNELS`(4브랜드)만 잡아 레노덤 프로페셔널/네오스트라타/크리스티나 등의 공식몰 매출이 누락되던 문제. `channelGroup="자사 공식몰"`(바크로하우스 메인몰 제외) 기준으로 변경해 B2C 탭과 동일하게 전 브랜드 집계. 공식몰+바크로하우스 목표 이중 매칭도 함께 해소
- **바크로하우스 스마트스토어 B2C 공식몰 일관 집계** — YTD 차트/메인 대시보드/목표 매칭에서 "바크로하우스 스마트스토어"가 바크로하우스로 묶여 B2C에서 빠지던 불일치 수정. 4개 로직을 메인 `바크로하우스` 채널만 분리하도록 통일(스마트스토어는 B2C 자사 공식몰로 집계)
- **에이블리 → 종합몰 분류** — 신규 채널 에이블리를 B2C 종합몰 그룹에 추가(`config/mappings.ts`)
- **월별 매출 추이 차트에 달성률 표기** — `YearToDateChart` 각 월 라벨 아래 그 달 달성률(스택 실적 합 / 그 달 목표)을 색상 구분(녹색 95%↑ / 주황 70~95% / 빨강 70%↓)으로 표기. `monthlyTargets`를 넘기는 전 탭 자동 적용
- **관리자 데이터 새로고침** — BigQuery 인메모리 캐시(FactCube) 무효화. 헤더 인쇄 버튼 우측 관리자 전용 버튼(세션 기반, `config/access.ts`의 `isAdmin`/`ADMIN_EMAILS`)으로 캐시 비움 → 화면 재로드. 토큰 보호 엔드포인트 `/api/refresh`(스크립트/자동화용)도 제공. `bigquery-provider.invalidateCache()` → `load.ts` 재노출

### v5.7 (2026-06-01)
B2B종합(영업사원별 통합) 탭 신설 + 접근 권한 재편 + 핵심변동 거래처 중심화 + 접속 로깅.
- **B2B종합 탭(`/b2b-summary`) 신설** — 영업사원(내부 직원)별로 직거래처·대리점·링커·바크로하우스 4개 소스를 종합. 최상단 영업사원별 통합 실적 표(소스별 분해 + 합계 + 전월 대비) + 소스별 12개월 스택 추이 + 4개 소스별 상세 섹션(월별 추이 + 담당자별 실적). 바크로하우스 칼럼은 본사 직접 관리분 / 대리점·링커 경유분으로 구분. 집계는 `lib/salesRepSummary.ts`
- **영업사원 = 직원 + 링커 개념 도입** — 링커(외부 영업사원/회사, 예: Harinbeauty)는 `dealer` 필드에 직원과 섞여 등장. `config/mappings.ts`의 `LINKERS`로 링커→담당 직원 매핑 관리(`isLinker`/`linkerManager`). 링커·대리점·바크로하우스 매출은 담당 내부 직원에게 귀속
- **대리점 담당자 = 최신 날짜 딜러** — FactCube에 `customerToLatestDealer` 추가. 담당자 이관(예: 고명성→류현희) 시 한 대리점 매출이 옛 담당자로 쪼개지지 않고 최신 담당자 1명에게 귀속
- **접근 권한 전환** — 기존 비공개 B2B 탭을 전체 공개로, 신설 B2B종합 탭을 화이트리스트(9명) 비공개로 변경 (`config/access.ts` `RestrictedTab` → `/b2b-summary`)
- **기존 탭 거래처 중심 재편** — B2B·바크로하우스 탭에서 영업담당자별 섹션 제거(B2B종합으로 이관). B2B 핵심변동에서 영업사원 빅무버 제거 → 거래처 급증/급감·동면 복귀·신규·분기 절벽/핵심 이탈로 대체. 바크로하우스도 담당자 변동 제거 → 바크로하우스 채널 샵 거래처 정보로 대체
- **거래처 분석 탭 보정** — 거래처 상세 헤더에 담당딜러를 인디고 배지로 강조(최신 담당자 기준). "거래처를 선택하세요" 목록을 전체기간 누적 → 해당월 매출 기준으로 수정
- **헤더 탭 그룹 시각 구분** — 보고 메뉴군(종합~면세점)과 분석 메뉴군(브랜드/거래처/심층 분석·비매출 출고)을 구분선 + 인디고 액센트로 분리
- **로그인 이벤트 로깅** — Auth.js `events.signIn`으로 로그인 시 이메일·시각을 구조화 JSON으로 기록(Cloud Logging)

### v5.6 (2026-05-29)
매트릭스 2뎁스 재설계 + 비매출 출고 탭 신설 + B2B 접근 제한 + UX 보정.
- **종합탭 매트릭스를 2뎁스 좌우 카드로 재설계** — 좌측 1뎁스(브랜드 × 6채널대분류 — 해외영업/B2B/대리점/바크로하우스/B2C/면세점) + 우측 2뎁스(선택 채널의 거래처 Top10). 1뎁스 셀 클릭이 우측 채널을 드릴다운하고 좌측 디테일 패널을 동시 표시. 1뎁스에만 채널 YTD 목표/달성률 노출 — 거래처 단위로 채널 목표를 공유 표시하던 이전 버전의 의미 혼동 해소
- **바크로하우스 채널 분류 수정** — "바크로하우스 스마트스토어"를 1뎁스에서 B2C로 재분류(이전엔 바크로하우스 채널대분류에 묶여 있음). 바크로하우스 2뎁스는 메인 sales 거래처 대신 `bh_partner_sales`(파트너 추천 매출) 기준 파트너 Top10으로 교체
- **비매출 출고 상세 탭(`/non-revenue`) 신설** — 자동 인사이트(휴리스틱 4종) + KPI 4종(건수/수량/원가/원가비율) + 사업형태별 도넛+표 + 12개월 사업형태별 스택 추이 + 거래처 Top20 + 채널×사업형태 매트릭스 + 브랜드 가로 막대 + 제품 Top20. 종합탭 하단 1줄 요약 → 상세 탭 링크로 전환
- **B2B 탭 접근 제한** — `config/access.ts`에 이메일 화이트리스트(9명). 비인가자는 탭 옆에 자물쇠 아이콘 표시 + 클릭 시 "열람 권한이 없습니다" 카드. 다른 탭은 모두 공개 유지
- **콜드 스타트 로딩바 동작 수정** — `LoadingProgress`가 JS hydration 의존 → CSS keyframe(0→85% 25초 ease-out)으로 전환. HTML 도착 즉시 시각적 진행 시작, hydration 늦어져도 0%에 멈춰있다가 점프하던 문제 해결
- **매트릭스 sticky 버그 2종 수정** — 스크롤 시 sticky 브랜드 셀이 페이지 헤더를 가리던 z-index 충돌(z-10 → z-[1]), 가로 스크롤 시 카드 좌측 padding으로 셀이 겹쳐 보이던 문제(padding을 sticky 셀로 이동)

### v5.5 (2026-05-28)
임원 분석 강화 — 거래처 클릭 일관성 + YTD 차트 비교선 + 인사이트 노이즈 보정 + 브랜드×거래처 매트릭스.
- 종합탭에 **브랜드 × 거래처 매트릭스** 신규 카드 추가 — 행=브랜드 6개 / 열=YTD 상위 15 거래처 격자, 색상 4분면(파랑/초록/빨강/황색 — 전년 동기 ± × 채널 목표 ±) + 회색, 셀 클릭 시 매출 5종 + 채널 목표 달성률 + 색상 사유가 인라인 expand 패널로 펼쳐짐 (기존 핵심 변동 카드는 유지)
- 모든 카드·표·차트에서 거래처명 클릭 → 거래처 분석 탭으로 이동 일관 처리 (`CustomerLink`/`customerHref` 공통 컴포넌트, 워터폴/막대/도넛/트리맵 차트에 클릭 라우팅)
- YTD 월별 매출 추이 차트에 **월별 목표**(amber 다이아몬드 실선)와 **전년 동기 매출**(slate 원형 점선) 라인 오버레이 추가 — 9개 탭 일괄 적용
- 거래처·카테고리 핵심 변동 인사이트에 **YTD 누적 컨텍스트** 추가 + 월 변동과 YTD 방향이 반대일 때 severity 자동 격하 (단발성 노이즈를 info로) — 예: "면세점 월 -36% / YTD 누적 +61%" 같은 케이스를 critical 대신 info로 표시
- 종합탭 상위 거래처 5개 → 10개로 확장
- 차트 컴포넌트 머지 동작 버그 수정 — 브랜드/거래처 콤보박스 변경 시 툴팁이 옛 데이터를 표시하던 문제 (`notMerge=true` 기본값)

### v5.4 (2026-05-27)
종합 탭 임원 보고용 심플화 + 탭 통합.
- 종합 탭: YTD/워터폴/일별누적/도넛/제품테이블 제거, 전체+6채널(B2B/대리점/B2C/바크로하우스/면세점/수출) KPI 카드에 채널별 목표 달성률 추가, Top 10→5 거래처 축소, AccountHighlights를 변동 요약 카드 1개로 압축, 비매출 출고 1줄 요약
- 변동 분석 탭(/changes) 삭제 → 심층 분석 탭(/insights)에 고유 콘텐츠 통합 (10탭으로 축소)
- 심층 분석 탭: 다른 탭과 중복되는 거래처 4종 카드/YTD/채널그룹 상승하락 제거, 할인율 테이블에 전월 변화 열 병합, 신제품 비중% 추가, 이상치 거래(1억+) 이관
- 사람 코멘트 기능 및 `marked` 패키지 제거
- 목표달성 탭 월별 매출 추이 스택 차트에 대리점·바크로하우스 분리 표시

### v5.3 (2026-05-27)
초기 로딩 UX 개선.
- 레이아웃에서 BigQuery 데이터 로딩을 Suspense로 분리 → 콜드 스타트 시 빈 화면 대신 헤더+스켈레톤 즉시 표시
- `LoadingProgress` 프로그레스 바 컴포넌트 추가 (단계별 상태 메시지 + %)
- `MonthSelectLoader` 서버 컴포넌트 분리

### v5.2 (2026-05-27)
전 탭 제품 분석 + 브랜드 탭 고도화.
- 6개 채널탭(B2C/B2B/대리점/면세점/해외영업/바크로하우스)에 상위 20 제품 테이블 추가 (이번달·전월비교·올해 누적)
- `TopProductsTable` 공유 컴포넌트 + `topNProductsEnhanced()` 함수 (YTD 포함)
- 브랜드 분석 탭: 수출 카테고리 반영, 카테고리별 목표 달성률 테이블, 채널 상세 테이블 추가

### v5.1 (2026-05-27)
B2C·대리점 탭 고도화 + FactCube 버그 수정.
- 목표달성 탭 기간별(월/분기/반기/연간) 매트릭스 + 상세 대시보드
- B2B·대리점 탭에 영업사원/대리점별 월간 목표 달성 현황 (`dealer_targets` BigQuery 테이블)
- B2C 탭: 자사 공식몰 채널별·소호몰 브랜드별·임직원패밀리 테이블 및 12개월 스택 차트
- 대리점 탭: 모든 차트를 대리점별 스택으로 변환 + 도넛 차트 + 브랜드×대리점 매트릭스
- FactCube B2B 딜러 차원에서 대리점 매출 제외 (b2bCustomerType !== "대리점")
- B2C 탭에서 바크로하우스 완전 제외 (별도 탭 분리, 스마트스토어는 유지)
- B2C 목표 합계에 기타 채널 추가, 종합몰 수수료율 컬럼 제거

### v5.0 (2026-05)
탭 구조 개편 (9 → 11탭).
- 해외영업(`/export`) 탭 신설 — 국가별 매출, 12개월 추이, 국가×브랜드 매트릭스, 목표 달성
- 대리점(`/agencies`) 탭 신설 — 거래처별 실적, 브랜드 분해, 신규/이탈 거래처
- B2B 탭에서 대리점 거래처 완전 분리 (영업사원 실적 포함)
- 종합 탭에 수출 카테고리 추가 (B2B/B2C/면세점/수출 4대 카테고리)
- 목표 데이터 해외(수출) division 활성화
- FactCube에 `customerToB2bType` 인덱스 추가

### v4.1 (2026-05)
Cloudflare Workers → **Google Cloud Run** 전환.
- Workers 무료 CPU 10ms 제한으로 Next.js SSR 불가 → Cloud Run 전환
- Cloudflare KV + sync 스크립트 전부 제거
- BigQuery 직접 쿼리 → FactCube 인메모리 빌드 → 모듈 캐시
- Docker (`output: "standalone"`) + Dockerfile
- `sales.csv` 삭제 — BigQuery가 유일한 데이터 소스
- `target.csv` → BigQuery `targets` 테이블 이전 완료 + CSV 삭제

### v4.0 (2026-05)
CSV → BigQuery 데이터 소스 전환.
- Google Workspace 인증 추가 (Auth.js v5 + Google OAuth)
- 비동기 DataProvider 패턴 (`lib/providers/`)
- `lib/parsers.ts` 분리 (순수 파싱 함수)
- Cloudflare Workers 배포 시도 (CPU 제한으로 v4.1에서 Cloud Run으로 재전환)

### v3.0 (2026-05)
거래처/딜러 심층 분석 + 구조 개선. 커밋 `4a20046`.
- `/accounts` 탭 신규 — 동면 복귀 / 분기 절벽 / 상실된 핵심 거래처
- 모든 탭 상단에 자동 인사이트 (`lib/tabInsights.ts`, 휴리스틱 기반)
- 팩트 큐브 (`lib/facts.ts`) — 모든 차원 사전 집계
- 모든 라우트 `loading.tsx` 스켈레톤

### v2.0
- 9개 탭 체계 확립 (v5.0에서 11개로 확장)
- `target.csv` 통합 (목표 vs 실적)
- 한국어 라벨 전환
- 변화 요인 워터폴 차트

### v1.0
- 6개 탭 초기 빌드

---

## 최근 수정 (2026-06-10)

| 커밋 | 내용 |
|---|---|
| `9b81381` | B2B탭 신규거래처 위에 이번달 상위 거래처 20개 추가(대리점 제외) |
| `cb3efaf` | 연 목표 진도 채널별 항상 노출 + 작년 대비 페이스 추가 |
| `bb88217` | 목표달성탭 연 목표 진도 카드에 채널별 펼침 추가 |

## 최근 수정 (2026-06-08)

| 커밋 | 내용 |
|---|---|
| `4bae817` | 종합탭 채널별 카드에 전년 동월 대비 매출 추가 |
| `a88ec25` | 종합탭 이번달 상위 거래처 10→20개로 확대 |
| `172e45b` | 누적 실적 패널에 전년 동기 비교 추가 |
| `fd317e0` | 목표 달성 상태 구간 세분화 — '미달'(70~85%) 추가 |

## 최근 수정 (2026-06-05)

| 커밋 | 내용 |
|---|---|
| `5ae8fda` | 영업사원별 상세 실적 페이지(`/sales-rep`) 신설 + B2B종합 영업사원 이름 전수 클릭 |
| `a861130` | 월별 매출 추이 차트 X축에 그 달 달성률 표기(색상 구분) |
| `a3e8c6d` | 목표달성 공식몰 기준 B2C 통일(channelGroup) + 에이블리 종합몰 분류 |
| `03e9cd5` | 바크로하우스 스마트스토어 B2C 공식몰 일관 집계 + 목표 이중매칭 해소 |
| `94e8e8d` | 관리자 데이터 새로고침 — BigQuery 인메모리 캐시 무효화(버튼 + `/api/refresh`) |

## 최근 수정 (2026-06-01)

| 커밋 | 내용 |
|---|---|
| `e09f90e` | 로그인 이벤트 로깅 (Auth.js events.signIn → Cloud Logging) |
| `ce148c4` | B2B종합 통합 실적 표 헤더 정렬 — 바크로하우스 그룹 헤더 제거 |
| `0c6889e` | 헤더 탭 그룹 시각 구분 (분석 메뉴군 구분선+인디고 액센트) |
| `1b5dd89` | 거래처 분석 담당딜러 배지 강조 + 거래처 선택 전 목록 해당월 매출 기준 |
| `b626b43` | B2B·바크로하우스 핵심변동에서 영업사원/담당자 제거 → 거래처 정보 |
| `3afa76b` | B2B종합 바크로하우스 직접/대리점 구분 + 대리점 담당자 최신값 |
| `7eeb869` | B2B종합 탭 신설 — 영업사원별 4개 소스 통합 실적 |

## 최근 수정 (2026-05-29)

| 커밋 | 내용 |
|---|---|
| `d571f3e` | 콜드 스타트 시 로딩바가 0%에 멈춰있던 문제 — CSS keyframe 기반으로 전환 |
| `6a79a86` | B2B 탭 이메일 화이트리스트 접근 제한 + 자물쇠 아이콘 |
| `26bb8e3` | 매트릭스 가로 스크롤 시 sticky 브랜드 셀 좌측 여백으로 셀 겹침 수정 |
| `4333c5a` | 비매출 출고 상세 분석 탭(`/non-revenue`) 신설 |
| `c493b65` | 바크로하우스 스마트스토어 B2C 재분류 + 바크로하우스 2뎁스를 파트너 매출 기준으로 교체 |
| `80ab28d` | 매트릭스 sticky 브랜드 셀이 페이지 헤더를 가리던 z-index 충돌 수정 |
| `54314be` | 종합탭 브랜드 매트릭스를 2뎁스 좌우 카드로 재설계 |

## 최근 수정 (2026-05-28)

| 커밋 | 내용 |
|---|---|
| `7d9c747` | 종합탭에 브랜드 × 거래처 매트릭스 신규 카드 추가 |
| `13609fd` | 카테고리 핵심 변동 인사이트에도 YTD 컨텍스트 + severity 격하 적용 |
| `959ec83` | 거래처 핵심 변동 인사이트에 YTD 누적 컨텍스트 + severity 자동 격하 |
| `c8e7db0` | YTD 월별 매출 추이 차트에 월별 목표·전년 동기 라인 오버레이 추가 |
| `d49fa6a` | 브랜드·거래처 변경 시 차트 툴팁이 옛 데이터로 표시되던 버그 수정 |
| `feb656f` | 종합탭 상위 거래처를 5개 → 10개로 확장 |
| `df9c98e` | 모든 카드·표·차트에서 거래처명 클릭 → 거래처 분석으로 이동 |

---

## 인프라 참고

### Cloud Run
- 서비스명: `baquero-sales-report`
- 리전: `asia-northeast3` (서울)
- 메모리: 2GiB (159k행 인메모리 로드에 필요)
- 배포: `git push` → Cloud Run 소스 기반 자동 빌드/배포
- 비용: 무료 등급 범위 내 (월 1회 보고용 대시보드)

### BigQuery
- 프로젝트: `citric-lead-457515-v2`
- 데이터셋: `dashboard_1`
- 매출 테이블: `매출통계` (영어 컬럼명 — `bigquery-provider.ts`에서 한국어로 매핑)
- 목표 테이블: `targets` (brand, division, customer_key, month, target_amount)
- 담당자 목표 테이블: `dealer_targets` (name, type, month, target)
- 매출 컬럼: `channel, date, order_number, product_name, product_code, quantity, net_sales, order_amount, discount_amount, commission, shipping_fee, settlement, dealer, client, client_type, cost, brand`
- 비용: 무료 등급 범위 내 (쿼리 ~32MB/회, 월 1TB 무료)

### 환경변수 (Cloud Run)
- `AUTH_SECRET` — Auth.js 토큰 서명
- `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` — Google OAuth
- `BQ_PROJECT_ID` — BigQuery 프로젝트 ID
- `BQ_DATASET` / `BQ_TABLE` — BigQuery 데이터셋/테이블명

---

## TODO

- [x] `target.csv` → BigQuery `targets` 테이블 이전 (배포 없이 목표 즉시 반영)
- [ ] `docs/plan.md` 내 데이터 흐름도 업데이트 (v4.0 Cloudflare KV 흐름 → v4.1 BigQuery 직접 쿼리)
