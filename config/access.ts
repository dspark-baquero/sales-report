...
const B2B_ALLOWED_EMAILS = new Set<string>([
...
export function canAccessB2B(email: string | null | undefined): boolean {
  if (!email) return false;
  return B2B_ALLOWED_EMAILS.has(email.toLowerCase());
}

// 탭별 접근 가능 여부 일괄 판정.
// 추후 다른 제한 탭 추가 시 여기에 매핑 추가.
export type RestrictedTab = "/b2b";

export function canAccessTab(tab: RestrictedTab, email: string | null | undefined): boolean {
  switch (tab) {
    case "/b2b":
      return canAccessB2B(email);
  }
}

export const RESTRICTED_TABS: RestrictedTab[] = ["/b2b"];
